...
for idx, d in enumerate(directions):
    ...
    for length in lengths:
        ...
        vel_avg_dir_trials = []
        for trial in ['b','c']:
            time, y, x , z = np.loadtxt('CFSMD_POLYLENGTH_unwrapped_pos_' + str(length) + '_' + d + "_"+trial+".txt", unpack=True)
            pos = y
            if dir == 'x':
                pos = x
            vel, std = calcVel(time,pos,binTime)
            vel_avg_dir_trials.append(vel)
            vels_std_dir.append(std) #append then flatten
            velLen = retVels(time, pos, binTime)
        #average over the trails
        vels_avg_dir.append(np.mean(vel_avg_dir_trials))
        #vels_std_dir = np.array(vel_std_dir).flatten()
    ns_per_ele =  (time[-1] - time[0] )/ len(time)
    ...
    print(vels_std_dir)
    #ax.errorbar(lengths,vels_avg_dir, yerr=np.array(vels_std_dir).flatten(), color = colors[idx])
...
plt.tight_layout()
plt.savefig("tmp__velocity_vs_length_y_cfsmd.pdf")
plt.show()
